/* ---------------------------------------------------------
   ...
   6부에서 달라진 것은 "수정" 버튼 하나입니다. 5부에서는 onEdit 을
   받아 App 의 editingId 를 바꿨지만, 이제 수정 모드는 주소가 갖고
   있으므로 /edit/3 으로 가는 Link 를 놓으면 그만입니다.
   --------------------------------------------------------- */
  ...
  if (error) {
    ...
    rows = students.map((student) => (
      ...
      <tr key={student.id}>
        ...
        <td>
          {/* 5부의 onEdit(student.id) 자리다. 이제는 주소만 바꾸면
                        App 의 Route 가 수정 폼을 그려 준다. */}
          <Link to={`/edit/${student.id}`} className="edit-btn">
            수정
          </Link>

          {/* onClick 에는 함수를 "넘겨야" 한다. onDelete(student.id) 라고
...
/* 받은 props 가 그대로면 다시 그리지 않는다(실습 5-13).
   students 가 바뀌지 않았는데 부모가 다시 그려질 때 이 표까지
   다시 그릴 까닭이 없다. */
